...
const Index = () => {
  ...
  const renderData = (category) => {
    ...
    return Data[0][category]
      .filter((item) => (selectedLocation === "Oshawa" ? item.oshawa : !item.oshawa))
      .map((item, index) => (
        <Link
        ...
        >
          <div className="relative">
            <Image src={item.image || "/images/logo.png"} alt={item.name} width={300} height={280} className="w-[100%] h-[280px] object-cover" />
          ...
          <div className="flex flex-col gap-3 pt-4 px-4 pb-4">
            <h3 className="text-gray-500 text-sm font-medium uppercase tracking-wide">{item.title}</h3>
            <p className="text-gray-800 font-bold text-lg" style={{ fontSize: Clamp(1, 1.25) }}>
              {item.name}
            </p>
  ...
  return (
    <>
      <Head>
        <title>Whitby Doctors at Valleyview Medical | Valleyview Medical Centre</title>
      </Head>

      <section className="relative bg-gradient-to-b from-green-50 to-white py-[100px] padding-x">
        {/* Header */}
        <div className="text-center max-w-4xl mx-auto mb-16">
          <span className="inline-block bg-green-100 text-green-700 text-[14px] md:text-[15px] px-4 py-1 rounded-full font-medium mb-4">Meet Our Team</span>
          ...
          <p className="text-gray-600 mt-4 text-[16px] md:text-[18px] leading-relaxed">Dedicated professionals committed to providing exceptional healthcare services</p>
        </div>
            ...
            {tabOptions.map((tab) => (
              <button
                ...
                className={`px-6 py-3 rounded-xl font-medium transition-all duration-300 ${
                  activeTab === tab.key ? "bg-white text-green-700 shadow-md" : "text-gray-600 hover:text-green-600 hover:bg-green-50"
                }`}
        ...
        {/* Staff Grid */}
        <div className="flex flex-wrap items-center gap-5 w-full">{renderData(activeTab)}</div>
      </section>
    </>
  );
};
